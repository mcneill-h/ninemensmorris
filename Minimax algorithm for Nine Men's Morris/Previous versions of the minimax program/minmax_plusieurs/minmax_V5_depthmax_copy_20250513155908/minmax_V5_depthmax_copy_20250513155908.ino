...
int bot[9] = { 1, 10, -2, -2, -2, -2, -2, -2, -2 };  //{ -2, 17, 27, 1, 11, -2, -2, -2, -2 }
int taille_liste_bot = 8;                            //donne le nombre d'éléments dans la liste bot

int player[9] = { -2, -2, -2, -2, -2, -2, -2, -2, -2 };  //{ 3, 2, 14, 15, -2, -2, -2, -2, -2 }
    ...
    if (in_beginning == true) {  //en debut de jeu qd les joueurs place les pieces
      ...
      for (int i = 0; i <= 27; i++) {
        if ((case_occupe(i) == false) && (i != 8) && (i != 9) && (i != 18) && (i != 19)) {
          ...
          if (killing == true) {
            ...
            for (int k = 0; k <= 8; k++) {  // pour toutes les possibilité où on suprime un des pions adverse
              if (player[k] != -2) {
                ...
                // est egal à 100 pour que ca soit tout de suite remplacé
                ...
                for (int l = 0; l <= 27; l++) {  //move possible de l'adversaire
                  if ((case_occupe(l) == false) && (l != 8) && (l != 9) && (l != 18) && (l != 19)) {
                    ...
                    if (global_alignement(l) == true) {  // dans le cas où il y a un killing
                      score_challenge = -2;
                    }
          ...
          //priorité: 1. à côté de la derniere piece placé //2. à côté d'une autre pièce placé prédédement par l'aversaire //3. au plus loin de la dernière piece que j'ai placé
          ...
          if (score > score_final) {
            ...
            int derniere_piece;             // trouve la derniere piece qui a été joué pas le joueur
            for (int j = 0; j <= 8; j++) {  // remplacer liste_global_alignement par la liste de player
              if (player[j] != -2) {
                derniere_piece = player[j];
              }
            }

            int interring = false; // détermine si c'est à côté en inter-ring
            if ((i%2 == 1)&&(MEME BRANCHE)) {  //seulement le cas qd le nombre est impair 
              interring = true;
            }

            if ((derniere_piece + 1 == i) || (derniere_piece - 1 == i)|| (interring == true)) {  // si il se trouve sur la meme ring



            } else {  //on choisit random
